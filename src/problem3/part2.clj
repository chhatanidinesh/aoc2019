...
(defn generate-points-in-direction
  [[x-start y-start] direction n]
  (condp = direction
    "U" (map (fn [v] [x-start (+ y-start v)] ) (range 1 (+ n 1)))
    "D" (map (fn [v] [x-start (- y-start v)] ) (range 1 (+ n 1)))
    "L" (map (fn [v] [(- x-start v) y-start] ) (range 1 (+ n 1)))
    "R" (map (fn [v] [(+ x-start v) y-start] ) (range 1 (+ n 1)))))


(defn get-all-points-of-line
  [l]
  (reduce
   (fn [acc point]
     (let [[direction dist] (re-seq   #"[R|U|L|D]|\d+" point)
           start (last acc)
           points (generate-points-in-direction
                   start
                   direction
